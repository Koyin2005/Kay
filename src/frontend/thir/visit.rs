...
pub trait Visitor<'a> : Sized{
    fn body(&self) -> &'a Body;
    fn visit_expr(&mut self, expr: &thir::Expr){ walk_expr(self, expr);}
    fn visit_block(&mut self, block : &thir::Block) { walk_block(self, block);}
    fn visit_arm(&mut self, arm: &thir::Arm) { walk_arm(self, arm);}
    fn visit_stmt(&mut self, stmt: &thir::Stmt) { walk_stmt(self, stmt);}
    fn visit_pattern(&mut self, pat: &thir::Pattern) { walk_pattern(self, pat);}
}


pub fn walk_expr<'thir,'a>(v : &mut impl Visitor<'a>, expr: &'thir thir::Expr){
    match &expr.kind{
        &ExprKind::Return(expr) => if let Some(expr) = expr {
            v.visit_expr(v.body().expr(expr));
        },
        ExprKind::Literal(_) | ExprKind::Var(_) | ExprKind::Constant(_,_) => (),
        &ExprKind::NeverToAny(expr) | &ExprKind::Field { receiver:expr, field:_, field_span:_ } | 
        &ExprKind::Unary(_, expr) | &ExprKind::Loop(expr) => 
            v.visit_expr(v.body().expr(expr)),
        &ExprKind::Assign(first,second) | &ExprKind::Index(first, second) | &ExprKind::Binary(_,first,second) => {
...
pub fn walk_pattern<'thir,'a>(v : &mut impl Visitor<'a>, pat: &'thir thir::Pattern){
    match &pat.kind{
        thir::PatternKind::Binding(..) | thir::PatternKind::Wilcard => (),
        thir::PatternKind::Case(..,fields) => fields.iter().for_each(|field|{
